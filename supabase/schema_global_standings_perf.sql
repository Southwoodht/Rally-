-- Rally — rank people on who they have actually beaten
--
...
-- falls back to the previous behaviour when they are absent.
--
-- Supersedes schema_global_standings_badloss.sql and everything before it.
-- On a fresh database this file alone is enough.
--
-- WHAT IT ADDS, AND WHY
--
-- Evidence used to mean one thing only: results against opponents at or
-- above your own level. Everything else was invisible. So Chris, 2-0-1, had
-- both wins ignored because they were against weaker players, and his single
-- loss counted — a winning record sitting near the bottom. Zaach, 2-0-0,
-- contributed nothing at all. And Charlie Easey, 6-3-10, was invisible in
-- both directions because he has no level of his own to compare against, so
-- his nineteen matches said nothing about him.
--
-- These five aggregates cover every match against an opponent who has a
-- level, and crucially they do NOT require the person themselves to have
-- one. That is the whole idea: how good somebody is should be a thing their
-- results say, not a thing they typed into a dropdown. Somebody unrated who
-- beats Intermediates has told us plenty.
--
--   rated_n          matches against a levelled opponent
--   opp_lv_sum       those opponents levels, summed - the average is the
--                    yardstick they were measured against
--   rated_res_sum    1 a win, 0.5 a draw, over the same matches
--   rated_share_sum  share of games taken, over the ones with a score
--   rated_share_n    how many of those had a usable score
--   rated_scored_res_sum  the plain results over just those scored ones, so
--                    the margin blend swaps like for like
--
-- The app turns them into a performance rating: the level you played like,
-- which is roughly the average level you faced, adjusted by how you did
-- against it. Weights live in src/lib/globalTable.ts as always — this
-- function reports facts and holds no opinion about them.
...
returns table (
  ...
  rated_n      int,     -- matches against an opponent who has a level
  opp_lv_sum   numeric, -- their levels, summed
  rated_res_sum numeric,-- 1 per win, 0.5 per draw, over those matches
  rated_share_sum numeric, -- share of games taken, over the scored ones
  rated_share_n int,    -- how many of them had a usable score
  rated_scored_res_sum numeric -- the 1/0.5/0 results over just those scored ones
)
language sql
stable
security definer
...
as $$
  ...
  agg as (
    select s.pkey,
      ...
      -- Everything below is measured against opponents who have a level,
      -- and deliberately does NOT need the person themselves to have one.
      -- That is the point: how good you are is a thing your results say, and
      -- an unrated player beating Intermediates has told us plenty.
      count(*) filter (where s.opp_lv is not null)::int as arated_n,
      coalesce(sum(s.opp_lv) filter (where s.opp_lv is not null), 0)::numeric as aopp_lv_sum,
      coalesce(sum(case s.res when 'W' then 1 when 'D' then 0.5 else 0 end)
               filter (where s.opp_lv is not null), 0)::numeric as arated_res_sum,
      coalesce(sum(s.my_share) filter (where s.opp_lv is not null and s.my_share is not null), 0)::numeric as arated_share_sum,
      count(*) filter (where s.opp_lv is not null and s.my_share is not null)::int as arated_share_n,
      coalesce(sum(case s.res when 'W' then 1 when 'D' then 0.5 else 0 end)
               filter (where s.opp_lv is not null and s.my_share is not null), 0)::numeric as arated_scored_res_sum
    from scored s
    group by s.pkey
  ),
  base as (
  ...
  select i.pkey,
         ...
         coalesce(a.arated_n, 0), coalesce(a.aopp_lv_sum, 0),
         coalesce(a.arated_res_sum, 0), coalesce(a.arated_share_sum, 0),
         coalesce(a.arated_share_n, 0), coalesce(a.arated_scored_res_sum, 0)
  from ident i
