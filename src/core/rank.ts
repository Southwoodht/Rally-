...
export function rankMaps(players, matches, elo, wdl) {
  ...
  matches.filter((m) => countsAsPlayed(m)).forEach((m) => { if (avgOpp[m.p1]) { avgOpp[m.p1].s += (elo[m.p2] ?? 0); avgOpp[m.p1].n++; } if (avgOpp[m.p2]) { avgOpp[m.p2].s += (elo[m.p1] ?? 0); avgOpp[m.p2].n++; } });
...
export function currentStreakOf(pid, matches) {
  const gs = matches.filter((m) => countsAsPlayed(m) && (m.p1 === pid || m.p2 === pid)).sort((a, b) => a.date - b.date);
  let s = 0;
