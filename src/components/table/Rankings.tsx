  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, alignItems: "stretch" }}>
        <Toggle on={mode === "overall"} onClick={() => onMode("overall")} label="Official" />
        ...
        return (
          <button key={p.id} onClick={() => onOpen(p.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 4px", borderBottom: "1px solid " + LINE, background: "transparent", border: "none", width: "100%", textAlign: "left", cursor: "pointer" }}>
            ...
            <div style={{ textAlign: "right", minWidth: 46 }}>
              <div style={{ fontFamily: mono, fontSize: mode === "record" ? 20 : 22, fontWeight: 700, color: leader ? BALL : CHALK, fontVariantNumeric: "tabular-nums" }}>{mode === "elo" ? rating : mode === "record" ? (r.gp ? recordStr(r) : "0-0") : (r.gp && official ? Math.round(official[p.id]) : "–")}</div>
              <div style={{ fontFamily: mono, fontSize: 9, color: MUTED, letterSpacing: 1, textTransform: "uppercase" }}>{mode === "elo" ? "elo" : mode === "record" ? "W–L" : "rating"}</div>
            </div>
          </button>
        );
      })}
