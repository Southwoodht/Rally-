      ...
      {clubs.map((c) => {
        ...
        return (
          <div key={c.id} style={{ marginBottom: 24 }}>
            ...
            {pending.length === 0 ? (
              <div style={{ fontFamily: body, fontSize: 13, color: MUTED, background: PANEL, border: "1px solid " + LINE, borderRadius: 10, padding: "14px" }}>No pending claims.</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {pending.map((t) => (
                  <div key={t.id} style={{ background: PANEL, border: "1px solid " + LINE, borderRadius: 10, padding: "12px 14px" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                      <button disabled={busyId === t.id} onClick={() => act(t.id, approveTrophy)} style={{ flex: 1, fontFamily: mono, fontSize: 11, textTransform: "uppercase", fontWeight: 700, padding: "8px 10px", borderRadius: 6, cursor: "pointer", border: "none", background: BALL, color: "#15352a" }}>Approve</button>
                      <button disabled={busyId === t.id} onClick={() => act(t.id, rejectTrophy)} style={{ flex: 1, fontFamily: mono, fontSize: 11, textTransform: "uppercase", fontWeight: 700, padding: "8px 10px", borderRadius: 6, cursor: "pointer", border: "1px solid " + CLAY, background: "transparent", color: CLAY }}>Reject</button>
                    </div>
                  </div>
                ))}
              </div>
