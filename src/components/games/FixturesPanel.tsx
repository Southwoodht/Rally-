...
import { PlayerIdentity, SurfaceCard } from "@/components/ui/Surfaces";
...
export function FixturesPanel({ fixtures, players, elo, matches, nameOf, onResolve, onBook }: any) {
  ...
  if (!total) {
    return (
      <SurfaceCard radius={18}>
        <div style={{ fontFamily: body, fontWeight: 500, fontSize: 15, color: FEED_TEXT_HI, marginBottom: 6 }}>No fixtures set up.</div>
  ...
  const openRow = (f: any) => { setOpen(open === f.id ? null : f.id); setWhenText(f.booked || ""); setScoreText(""); };
  const q = search.trim().toLowerCase();
  const shown = q ? fixtures.filter((f: any) => (nm(f.p1) + " " + nm(f.p2)).toLowerCase().includes(q)) : fixtures;
              ...
              <button onClick={() => openRow(f)} style={{ width: "100%", background: "transparent", border: "none", cursor: "pointer", textAlign: "left", padding: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <span style={{ flex: 1, minWidth: 0 }}>
                    <PlayerIdentity player={who(f.p1)} name={nm(f.p1)} size={0} nameSize={15} />
                  </span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 9 }}>
                  {f.booked && (
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: FEED_LIME, color: FEED_LIME_INK, borderRadius: 999, padding: "2px 9px", fontFamily: body, fontWeight: 500, fontSize: 11.5 }}>
                      <Calendar size={11} strokeWidth={2.2} />{f.booked}
                    </span>
                  )}
                  <span style={{ flex: 1 }} />
                  <span style={{ fontFamily: body, fontWeight: 400, fontSize: 12, color: FEED_LIME, flexShrink: 0 }}>
                    {open === f.id ? "Close" : "Book or enter result"}
                  </span>
                </div>
              </button>
                ...
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: "0.5px solid " + FEED_HAIRLINE }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: f.booked ? 8 : 16 }}>
                    <input
                      ...
                      placeholder="When? e.g. Saturday 2pm"
                      style={{ ...miniInput, fontFamily: body, fontSize: 14, background: FEED_RAISED, color: FEED_TEXT_HI, flex: 1, padding: "10px 12px", boxSizing: "border-box" as const }}
                    />
                    <button onClick={() => onBook(f.id, whenText.trim())} style={{ ...actionBtn(FEED_LIME, FEED_LIME_INK), flex: "0 0 auto", padding: "10px 16px" }}>Book</button>
                  </div>
                  {f.booked && (
                    <button
                      onClick={() => onBook(f.id, "")}
                  ...
                  <input
                    ...
                    style={{ ...miniInput, fontFamily: body, fontSize: 14, background: FEED_RAISED, color: FEED_TEXT_HI, width: "100%", marginBottom: 10, padding: "10px 12px", boxSizing: "border-box" as const }}
                  />
