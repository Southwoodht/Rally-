...
export function SettingsTab({ group, updateGroup, onRemovePlayer, fixtures, onGenerate, onClearFixtures, onAddFixture, onRemoveFixture, onLoadDemo, onClearResults, onImportHistoricalMatches, players, setPlayers, matches, flash }: any) {
  ...
  return (
    <div style={card}>
        ...
        {group?.season ? (
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: mono, fontSize: 9, textTransform: "uppercase", letterSpacing: 1, color: MUTED, marginBottom: 4 }}>Starts</div>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: mono, fontSize: 9, textTransform: "uppercase", letterSpacing: 1, color: MUTED, marginBottom: 4 }}>Ends (optional)</div>
      ...
      <Field label="Fixtures">
        ...
        {group?.fixtureMode === "roundrobin" ? (
          <div>
            ...
            <div style={{ fontFamily: mono, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.5, color: BALL, marginBottom: 8 }}>1 · Auto — everyone plays everyone</div>
            ...
            <div style={{ fontFamily: mono, fontSize: 10, textTransform: "uppercase", letterSpacing: 1.5, color: BALL, marginBottom: 8 }}>2 · Or add one specific match</div>
      ...
      <div style={{ borderTop: "1px solid " + LINE, marginTop: 6, paddingTop: 16 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ fontFamily: mono, fontSize: 10, color: MUTED, textTransform: "uppercase", letterSpacing: 1.5 }}>Players & pictures</div>
          <button onClick={() => setGuideOpen(true)} style={{ fontFamily: mono, fontSize: 10, letterSpacing: 1, textTransform: "uppercase", color: BALL, background: "transparent", border: "1px solid " + LINE, borderRadius: 6, padding: "5px 9px", cursor: "pointer" }}>What's my level?</button>
        </div>
        ...
        {players.map((p) => {
          ...
          return (
            <div key={p.id} style={{ padding: "10px 0", borderTop: "1px solid " + LINE }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                ...
                {confirmRemove === p.id ? (
                  <span style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => remove(p.id)} style={{ fontFamily: mono, fontSize: 10, color: COURT, background: CLAY, border: "none", borderRadius: 5, padding: "4px 8px", cursor: "pointer", textTransform: "uppercase", fontWeight: 700 }}>{played > 0 ? "Remove + " + played + " games" : "Confirm"}</button>
                    <button onClick={() => setConfirmRemove(null)} style={{ fontFamily: mono, fontSize: 10, color: MUTED, background: "transparent", border: "1px solid " + LINE, borderRadius: 5, padding: "4px 8px", cursor: "pointer", textTransform: "uppercase" }}>Keep</button>
                  </span>
                ) : (
                  <button onClick={() => setConfirmRemove(p.id)} style={{ fontFamily: mono, fontSize: 10, color: CLAY, background: "transparent", border: "1px solid " + LINE, borderRadius: 5, padding: "4px 8px", cursor: "pointer", textTransform: "uppercase" }}>Remove</button>
                )}
              </div>
              {avOpen === p.id && (
              ...
              <button onClick={() => setTlOpen(tlOpen === p.id ? null : p.id)} style={{ marginTop: 8, fontFamily: mono, fontSize: 10, letterSpacing: 1, textTransform: "uppercase", color: BALL, background: "transparent", border: "1px solid " + LINE, borderRadius: 6, padding: "5px 9px", cursor: "pointer" }}>Level timeline{p.levelHistory?.length ? " (" + p.levelHistory.length + ")" : ""}</button>
      ...
      <div style={{ borderTop: "1px solid " + LINE, marginTop: 6, paddingTop: 16 }}>
        <div style={{ fontFamily: mono, fontSize: 10, color: MUTED, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 10 }}>Data</div>
