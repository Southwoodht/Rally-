...
import { FEED_OVERLAY, BALL, CHALK, CLAY, COURT, LINE, MUTED, body, input } from "@/lib/theme";
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: FEED_OVERLAY, display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 95 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: COURT, width: "100%", maxWidth: 620, maxHeight: "88vh", overflowY: "auto", borderTopLeftRadius: 26, borderTopRightRadius: 26, border: "none", padding: "20px 18px 40px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          <div style={{ fontFamily: body, fontSize: 11, letterSpacing: 0.8, textTransform: "uppercase", color: MUTED }}>Claim a trophy</div>
          ...
          <Field label="Club">
            {!addingClub && clubs.length > 0 && (
              <>
                ...
                <button onClick={() => setAddingClub(true)} style={{ background: "transparent", border: "none", color: BALL, fontFamily: body, fontSize: 11, cursor: "pointer", textTransform: "uppercase", letterSpacing: 0.5 }}>+ Add a different club</button>
              </>
            )}
            {addingClub && (
              <>
                ...
                {clubs.length > 0 && <button onClick={() => setAddingClub(false)} style={{ background: "transparent", border: "none", color: MUTED, fontFamily: body, fontSize: 11, cursor: "pointer", textTransform: "uppercase", letterSpacing: 0.5, marginTop: 6 }}>Cancel — pick an existing club</button>}
